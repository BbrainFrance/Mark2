 ...
 */
function trimMessagesToFit(messages, maxTokens = 80000) {
...
function truncateToolContent(content) {
  ...
  return content.map(block => {
      ...
      if (inputStr.length > MAX_TOOL_CONTENT) {
        ...
        for (const [key, val] of Object.entries(block.input)) {
          if (typeof val === 'string' && val.length > 500) {
            if (block.name === 'write_file' && key === 'content') {
              truncatedInput[key] = `[fichier ecrit: ${val.length} caracteres - utilise read_file pour voir le contenu actuel]`;
            } else if (block.name === 'generate_document' || block.name === 'generate_pdf') {
              truncatedInput[key] = `[document genere: ${val.length} caracteres]`;
            } else {
              truncatedInput[key] = val.substring(0, 800) + `\n... [${val.length} caracteres au total]`;
            }
          } else {
    ...
    if (block.type === 'tool_result' && typeof block.content === 'string' && block.content.length > MAX_TOOL_CONTENT) {
      return { ...block, content: block.content.substring(0, MAX_TOOL_CONTENT) + `\n... [${block.content.length} caracteres au total]` };
    }
    return block;
  });
}

async function chat({ system, messages, tools = [], executeTool, model, maxTokens = 4096, maxToolRounds = 40 }) {
  ...
  while (round < maxToolRounds) {
    ...
    try {
      ...
      if (apiErr.status === 400 && apiErr.message && apiErr.message.includes('prompt is too long')) {
        console.log(`[Anthropic] Prompt trop long, trim agressif et retry...`);
        const trimmed = trimMessagesToFit(workingMessages, 40000);
        workingMessages.length = 0;
        workingMessages.push(...trimmed);
        apiParams.messages = workingMessages;
        response = await anthropic.messages.create(apiParams);
      } else {
        throw apiErr;
      }
    ...
    if (totalTokens > 100000) {
      const trimmed = trimMessagesToFit(workingMessages, 80000);
      workingMessages.length = 0;
      workingMessages.push(...trimmed);
    }
  ...
  if (round >= maxToolRounds) {
    console.warn(`[Anthropic] LIMITE ATTEINTE: ${maxToolRounds} rounds d'outils. L'agent pourrait halluciner les actions suivantes.`);
    finalResponse += '\n\n⚠️ LIMITE DE ROUNDS ATTEINTE — Je ne peux plus executer d\'outils. Les actions restantes doivent etre effectuees dans un nouveau message.';
  }
